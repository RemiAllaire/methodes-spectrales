% Paramètres
N = 500;                 % Nombre de fonctions de base
...
j = 0:N;
DiagCentre = (1 - beta^4)/(4*beta^2)*(2.*j + 1) + 0.5*beta^2*(1 + 2.*j);


Bout = [[Diag1,0,0];DiagCentre;[0,0,Diag1]]';
...
% Diagonalisation
H = spdiags(Bout,[-2,0,2],N+1,N+1);
[V, D] = eigs(H, round(N/3), 'smallestreal');
energies2 = diag(D);

exact = (0.5 + 0:round(N/3))';

erreur = abs(energies2-exact);
